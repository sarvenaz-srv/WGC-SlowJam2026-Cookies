...
## The line of dialogue this NPC says when the player first reaches them.
...
func _on_body_entered(body: Node2D) -> void:
	...
	# Let the NPC offer its gift, if it hasn't been given yet.
	if not gift_given:
		player_in_zone = body
		if prompt_label:
			var hint := "\n\n[Press E to accept]"
			if dialogue_text != "":
				prompt_label.text = dialogue_text + hint
			else:
